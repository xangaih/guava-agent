...
STATUS_PATH = Path(__file__).resolve().parent.parent / "concierge_call_status.json"
TRANSCRIPT_LIMIT = 50

_DEFAULT: dict[str, Any] = {"active_call": False, "trip_id": None, "transcript": []}
...
def call_started():
    _write({"active_call": True, "trip_id": None, "transcript": []})
...
def get_status() -> dict:
    data = _read()
    return {"active_call": data.get("active_call", False), "trip_id": data.get("trip_id")}
